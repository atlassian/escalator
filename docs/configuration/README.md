...
Specific node groups are selected through a label selector: `label_key` and `label_value`. Nodes will need to be
labeled to match the `nodegroups.yml` configuration file for Escalator to consider it in calculations. You can read more
about Labels and Selectors [here](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/).

In the example below, any nodes labeled with `"customer": "shared"` will be considered in Escalator calculations.
...
Replace `customer=shared` with your node label key-value.
